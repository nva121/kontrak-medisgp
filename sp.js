...
export default async function handler(req, res) {
  ...
  try {
    const { type, namaP, jabatanP, namaA, jabatanA, alasan } = req.body || {};
...
**( ${namaP} )**`;
    ...
    // Kirim ke Discord (pake content + embed: content kasih ping, embed kasih visual hierarchy)
    const payload = {
      content: `📨 **${title}** diterbitkan untuk **${namaA}** (${jabatanA})`,
      embeds: [{
        description: body,
        color,
        timestamp: new Date().toISOString(),
        footer: { text: 'Sistem Surat Peringatan • Rumah Sakit Garuda Prime' }
      }]
    };
